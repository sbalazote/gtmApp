			...
			if (productDetails.length > 0) {
				if($("#affiliateInput").val() != ""){
					...
					$.ajax({
						...
						async: true,
						success: function(response) {
							myReload("success", "Se ha registrado la solicitud de abastecimiento n�mero: " + response.id);
						},
						error: function(jqXHR, textStatus, errorThrown) {
							myGenericError();
						}
			...
			if (existsAffiliate()) {
				...
				$.ajax({
					...
					success: function(response) {
						...
						$('#affiliateInput').select2("data", {id: response.id, text: response.code + ' - ' + response.surname + ' ' + response.name});
					},
					error: function(jqXHR, textStatus, errorThrown) {
