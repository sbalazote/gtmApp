...
<form id="destructionModalForm" action="" onsubmit="return false;">
		...
		<div class="modal-dialog" style="width: 40%">
			<div class="modal-content">
						...
						<div class="col-md-12 col-lg-12 form-group">
							...
							<select id="destructionConceptInput" name="destructionConcept" class="form-control" data-placeholder="<spring:message code='common.select.option'/>" autofocus>
								...
								<c:forEach items="${destructionConcepts}" var="concept">
									<option value="${concept.id}"  ${conceptId == concept.id ? 'selected' : ''}><c:out value="${concept.code}"></c:out> - <c:out
											value="${concept.description}"></c:out></option>
								</c:forEach>
							</select>
						</div>
					</div>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" id="cancelDestruction"  data-dismiss="modal"><spring:message code="common.no"/></button>
					<button type="button" class="btn btn-primary" id="destructionAccept"><spring:message code="common.yes"/></button>
				</div>
			</div>
		</div>
	</div>
