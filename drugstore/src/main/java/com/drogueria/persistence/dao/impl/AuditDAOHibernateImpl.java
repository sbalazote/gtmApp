	...
	public AuditResultDTO getAudit(Integer productId, String serialNumber) {
        ...
        if (productId == null) {
            sentence = "select distinct a.* from audit as a, delivery_note_detail as dnd, supplying_detail as sd where (a.role_id = "
                    ...
                    + "' and a.operation_id = dnd.delivery_note_id and dnd.supplying_detail_id = sd.id and dnd.order_detail_id is null) or (a.role_id = "
                    + RoleOperation.DELIVERY_NOTE_CANCELLATION.getId() + " and sd.serial_number = '" + serialNumber
                    + "' and a.operation_id = dnd.delivery_note_id and dnd.supplying_detail_id = sd.id and dnd.order_detail_id is null) order by a.`date` desc";
        }
        if (productId != null) {
            sentence = "select distinct a.* from audit as a, delivery_note_detail as dnd, supplying_detail as sd where (a.role_id = "
